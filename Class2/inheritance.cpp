...
protected:
  static int start_id;  //A static data member is created only one instance for the entire program
  const int ID; //A constant data member cannot change its value 
  string fn;
  string ln;
  double salary;
public:
  Employee(string f = "unknown", string l = "unknown"); //A constant data member needs to be initialized in the func header
  void showInfo();
  static int getStartId(); //prototype
};

int Employee::start_id = 1000; //initialize a static data member outside the class definition. You need clasEmployee::static_data_member


//You can set default values in the func header of the constructor 
Employee::Employee(string f, string l): ID(start_id) //A constant data member needs to be initialized in the func header
{
    fn = f;
    ln = l;
    salary = 40000;
...
void Employee::showInfo()
{
    ...
    cout << ID << "(ID) " << fn << "(fn) " << ln << "(ln) " << salary << "(salary)" << endl;
}

int Employee::getStartId() //This function needs to be implemented after start_id is initialized.
{
  return start_id;
}
...
//################################ Manager class ####################################### 
class Manager : public Employee {  //Manager is a inherited class of the Employee class (called base class)
private:
  int groupNum;
  int rank;//1, 2, or 3
public:
  Manager(string f = "unknown", string l = "unknown", int r = 1, int gn = 0) : Employee(f, l)
  {
    rank = r;
    groupNum = gn;
    setSalary();
...
public:
  ...
  SE(string f = "unknown", string l = "unknown", int yr = 0, int gn = 0) : Employee(f, l)
  {
    exp_yr = yr;
    groupNum = gn;
    setSalary();
  ...
  /* The following getCount was added to LL class
  int LL<T>::getCount() const
  {
    return count;
  }
  */
  ...
  void showInfo()
  {
    ...
    Employee::showInfo();
    //cout << ID << "(ID) " << fn << "(fn) " << ln << "(ln) " << salary << "(salary) " << endl; //The protected members of the base class can be accessed from the inherited class. An inherited class cannot access private members.
...
int main()
{
  ...
  Manager m;  //new operator returns the address of the manager object. The default values will be used.
  SE se("Mike", "Smith", 3, 10);
  Employee e("Kathy", "Ross"); //You can make an object from the base class too
