...
*******************************************/
template<typename T>
int getHighest(const T array[], int SIZE) {
    T largest = array[0];
    int res = 0;
...
*******************************************/
template<typename T>
int getLowest(const T array[], int SIZE) {
    int res = 0;
    T smallest = array[0];
    for (int i = 1; i < SIZE; i++) {
        if(smallest > array[i]) { 
...
T getAvg(const S array[], int SIZE) {
    T avg = 0;
    for (int i = 0; i < SIZE; i++) {
        avg += array[i];
    }
...
int find(const T array[], int size, const T1& key) {
    for (int i = 0 ; i < size; i++)
        if(array[i] == key) return i; // to match the paramters
    return -1; // return -1 if nothing was found
}
template<typename T, typename T1>
bool remove(T array[], int size, const T1& key) {
